...
def Main(file_name, NUMBER_PIXPICS, MAX_IM, RESOLUTION_FACTOR, MOST_USED_THRESHOLD):
    ...
    mainpicture = ReadPicture(main_pic_dir)
    height1, width1 = len(mainpicture), len(mainpicture[0])

    # Determine size of pixel pictures
    dim_pix, amount_height, amount_width = SizePixelPic(height1, width1, NUMBER_PIXPICS)
    ...
    reconstructed = Matrix(amount_height * dim_pix, amount_width * dim_pix, [np.uint8(0), np.uint8(0), np.uint8(0)])

    # Create Current error matrix
    current_error = Matrix(amount_height, amount_width, None)
    ...
    for im_num in tqdm(range(1, MAX_IM+1)):
        ...
        pixel_pic = ReadPicture(pixel_pics_dir + "Pixel_Im ("+ str(im_num) + ").jpg")
        ...
        error = Error(mainpicture, pixel_pic, amount_height, amount_width, dim_pix)
    ...
    zero_images, counter, x_axis = ZeroImages(image_count, MAX_IM)
    
    # Quantise the imagess that are used a lot
    max_images = MaxImages(counter, x_axis, MOST_USED_THRESHOLD)
...
def Error(picture1, picture2, height, width, dim_pix):
    error = []
...
def Matrix(height, width, values):
    matrix = []
...
def SizePixelPic(height, width, NUMBER_PIXPICS):
...
def ZeroImages(image_count, MAX_IM):
    zero_images = []
    x_axis = []
    counter = []
...
def MaxImages(counter, x_axis, MOST_USED_THRESHOLD):
    max_images = []
